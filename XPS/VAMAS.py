...
class VAMASExperiment:
    ...
    def read_from_file(self):
        ...
        if (self.experiment_mode.upper() == 'MAP' or
            ...
            self.number_of_spectral_regions = int(next(lines).strip())

        if (self.experiment_mode.upper() == 'MAP' or
            self.experiment_mode.upper() == 'MAPDP'):
            self.number_of_analysis_positions = int(next(lines).strip())
            self.number_of_discrete_x_coordinates_available_in_full_map = int(next(lines).strip())
            self.number_of_discrete_y_coordinates_available_in_full_map = int(next(lines).strip())

        self.number_of_experimental_variables = int(next(lines).strip())

        for i in range(self.number_of_experimental_variables):
            self.experimental_variable_labels.append(next(lines).strip())
            self.experimental_variable_units.append(next(lines).strip())

        self.number_of_entries_in_parameter_inclusion_or_exclusion_list = int(next(lines).strip())

        for i in range(abs(self.number_of_entries_in_parameter_inclusion_or_exclusion_list)):
            self.parameter_inclusion_or_exclusion_prefix_numbers.append(int(next(lines).strip()))

        self.number_of_manually_entered_items_in_block = int(next(lines).strip())

        for i in range(self.number_of_manually_entered_items_in_block):
            self.prefix_numbers_of_manually_entered_items.append(int(next(lines).strip()))

        self.number_of_future_upgrade_experiment_entries = int(next(lines).strip())

        self.number_of_future_upgrade_block_entries = int(next(lines).strip())

        for i in range(self.number_of_future_upgrade_block_entries):
            self.future_upgrade_experiment_entries.append(next(lines).strip())

        self.number_of_blocks = int(next(lines).strip())

        for i in range(self.number_of_blocks):
            block = VAMASBlock()
            block.block_identifier = next(lines).strip()
            block.sample_identifier = next(lines).strip()
            # 1
            ...
            # 7
            block.number_of_hours_in_advance_of_greenwich_mean_time = int(next(lines).strip())

            # 8
            block.number_of_lines_in_block_comment = int(next(lines).strip())
            for i in range(block.number_of_lines_in_block_comment):
                block.comment = block.comment + next(lines)

            # 9
            block.technique = next(lines).strip()

            # 10
            if (self.experiment_mode.upper() == 'MAP' or
                self.experiment_mode.upper() == 'MAPDP'):
                block.x_coordinate = int(next(lines).strip())
                block.y_coordinate = int(next(lines).strip())

            # 11
            for i in range(self.number_of_experimental_variables):
                block.values_of_experimental_variables.append(float(next(lines).strip()))

            # 12
            block.analysis_source_label = next(lines).strip()
            ...
            if (self.experiment_mode.upper() == 'MAPDP' or
                ...
                block.sputtering_ion_or_atomic_number = next(lines).strip()
                block.number_of_atoms_in_sputtering_ion_or_atom_particle = int(next(lines).strip())
                block.sputtering_ion_or_atom_charge_sign_and_number = int(next(lines).strip())

            # 14
            block.analysis_source_characteristic_energy = float(next(lines).strip())

            # 15
            block.analysis_source_strength = float(next(lines).strip())

            # 16
            block.analysis_source_beam_width_x = float(next(lines).strip())
            block.analysis_source_beam_width_y = float(next(lines).strip())
            ...
            if (self.experiment_mode.upper() == 'MAP' or
                ...
                block.field_of_view_x = float(next(lines).strip())
                block.field_of_view_y = float(next(lines).strip())

            # 18
            if (self.experiment_mode.upper() == 'MAPSV' or
                self.experiment_mode.upper() == 'MAPSVDP' or
                self.experiment_mode.upper() == 'SEM'):
                block.first_linescan_start_x_coordinate = int(next(lines).strip())
                block.first_linescan_start_y_coordinate = int(next(lines).strip())
                block.first_linescan_finish_x_coordinate = int(next(lines).strip())
                block.first_linescan_finish_y_coordinate = int(next(lines).strip())
                block.last_linescan_finish_x_coordinate = int(next(lines).strip())
                block.last_linescan_finish_y_coordinate = int(next(lines).strip())

            # 19
            block.analysis_source_polar_angle_of_incidence = float(next(lines).strip())

            # 20
            block.analysis_source_azimuth = float(next(lines).strip())

            # 21
            block.analyser_mode = next(lines).strip()

            # 22
            block.analyser_pass_energy_of_retard_ratio_or_mass_resolution = float(next(lines).strip())

            # 23
            if (block.technique.upper() == 'AES DIFF'):
                block.differential_width = float(next(lines).strip())

            # 24
            block.magnification_of_analyser_transfer_lens = float(next(lines).strip())

            # 25
            block.analyser_work_function_or_acceptance_energy_of_atom_or_ion = float(next(lines).strip())

            # 26
            block.target_bias = float(next(lines).strip())

            # 27
            block.analysis_width_x = float(next(lines).strip())
            block.analysis_width_y = float(next(lines).strip())

            # 28
            block.analyser_axis_take_off_polar_angle = float(next(lines).strip())
            block.analyser_axis_take_off_azimuth = float(next(lines).strip())

            # 29
            block.species_label = next(lines).strip()

            # 30
            block.transition_or_charge_state_label = next(lines).strip()
            block.charge_of_detected_particle = int(next(lines).strip())

            # 31
            if (self.scan_mode.upper() == 'REGULAR'):
                block.abscissa_label = next(lines).strip()
                block.abscissa_units = next(lines).strip()
                block.abscissa_start = float(next(lines).strip())
                block.abscissa_increment = float(next(lines).strip())

            # 32
            block.number_of_corresponding_variables = int(next(lines).strip())
            for i in range(block.number_of_corresponding_variables):
                block.corresponding_variable_labels.append(next(lines).strip())
                block.corresponding_variable_units.append(next(lines).strip())

            # 33
            block.signal_mode = next(lines).strip()

            # 34
            block.signal_collection_time = float(next(lines).strip())

            # 35
            block.number_of_scans_to_compile_this_block = int(next(lines).strip())

            # 36
            block.signal_time_correction = float(next(lines).strip())
            ...
            if ( (block.technique.upper() == 'AES DIFF' or
                ...
                block.sputtering_source_energy = float(next(lines).strip())
                block.sputtering_source_beam_current = float(next(lines).strip())
                block.sputtering_source_width_x = float(next(lines).strip())
                block.sputtering_source_width_y = float(next(lines).strip())
                block.sputtering_source_polar_angle_of_incidence = float(next(lines).strip())
                block.sputtering_source_azimuth = float(next(lines).strip())
                block.sputtering_mode = next(lines).strip()

            # 38
            block.sample_normal_polar_angle_of_tilt = float(next(lines).strip())
            block.sample_normal_tilt_azimuth = float(next(lines).strip())

            # 39
            block.sample_rotation_angle = float(next(lines).strip())

            # 40
            block.number_of_additional_numerical_parameters = int(next(lines).strip())
            for i in range(block.number_of_additional_numerical_parameters):
                block.additional_numerical_parameter_labels.append(next(lines).strip())
                block.additional_numerical_parameter_units.append(next(lines).strip())
                block.additional_numerical_parameter_values.append(float(next(lines).strip()))

            # end numbered lines in block specification
            for i in range(self.number_of_future_upgrade_block_entries):
                block.future_block_entries.append(next(lines).strip())

            block.number_of_ordinate_values = int(next(lines).strip())

            variables = []
            for i in range(block.number_of_corresponding_variables):
                block.minimum_ordinate_values.append(float(next(lines).strip()))
                block.maximum_ordinate_values.append(float(next(lines).strip()))
                variables.append([])

            for i in range(int(block.number_of_ordinate_values/block.number_of_corresponding_variables)):
