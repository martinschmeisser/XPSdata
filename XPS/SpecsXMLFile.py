...
XML_TYPES = {
    'string': str, 'ulong': int, 'double': float, 'boolean': bool,
    'long': int,
}
...
def simple_convert(element):
    ...
    * **Any** is skipped and replaced with its content
    ...
    if element.text is None:
        out = None
    # parse array
    elif '\n' in element.text and element.tag in list(ARRAY_TYPES.keys()):
        out = np.fromstring(element.text, dtype=ARRAY_TYPES[element.tag],
                            sep='\n')
    # parse simple type
    elif element.tag in list(XML_TYPES.keys()):
...
class SpecsXMLFile(list):
    ...
    def __init__(self, filepath, encoding=None):
        ...
        super(SpecsXMLFile, self).__init__()
...
class Region(RegionBase):
    ...
    * **region**: Contains information like, dwell_time, analysis_method,
    ...
    def __init__(self, xml):
        ...
        if self.analyzer_info['detectors'] is not None:
            self.mcd_num = len(self.analyzer_info['detectors'])
            self.detectors = self.analyzer_info['detectors']
    ...
    def y_avg_counts(self):
        ...
        _LOG.debug('Creating %i y_avg_counts values from %i scans',
